...
COMMITTED_OED_V3_ALPHA2_CONFIG = (
    ROOT
    / "configs"
    / "multi_candidate_formal_v1_oed_v3alpha2_diagnostic.json"
)
COMMITTED_OED_V3_ALPHA3_CONFIG = (
    ROOT
    / "configs"
...
class OEDConfigTest(unittest.TestCase):
    ...
    def test_committed_v3alpha2_keeps_bounded_reveal_domain(self) -> None:
        config = load_oed_config(COMMITTED_OED_V3_ALPHA2_CONFIG)
        self.assertEqual(
            "multi-candidate-formal-v1-oed-v3alpha2-diagnostic",
            config.controller_id,
        )
        self.assertEqual(4, config.max_iterations)
        self.assertEqual(
    ...
    def test_committed_v3alpha3_keeps_bounded_reveal_domain(self) -> None:
        config = load_oed_config(COMMITTED_OED_V3_ALPHA3_CONFIG)
        self.assertEqual(
            "multi-candidate-formal-v1-oed-v3alpha3-diagnostic",
            config.controller_id,
        )
        self.assertEqual(4, config.max_iterations)
        self.assertEqual(
...
class OEDReplayTest(unittest.TestCase):
    ...
    def test_v3alpha3_replay_exposes_direct_utility_certificate(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            output = root / "oed-v3alpha3-output"
            run_oed_replay(
                _oed_config(root),
                awm_path,
                oracle_path,
            ...
            self.assertIsNotNone(
                candidate["paired_gain_normalized_utility_mean"]
            )
            self.assertEqual(
                "diagnostic-only-not-part-of-primary-confidence-family",
                candidate["paired_gain_component_interval_role"],
            )
            evaluation = json.loads(
                (output / "oed_evaluation.json").read_text()
            )
            self.assertEqual(
                "pathfinder.oed-replay/v3alpha3",
                evaluation["schema_version"],
            )

    def test_v3alpha2_replay_exposes_cluster_mean_audit_fields(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            oracle_path = _oracle_config(root)
            awm_path = _awm_config(
            ...
            output = root / "oed-v3alpha2-output"
            run_oed_replay(
                _oed_config(root),
                awm_path,
                oracle_path,
            ...
            self.assertEqual(
                [0, 1],
                candidate["paired_gain_within_cluster_repetition_ids"],
            )
            self.assertIsNotNone(
                candidate[
            ...
            self.assertEqual(
                "pathfinder.oed-replay/v3alpha2",
                evaluation["schema_version"],
            )
            self.assertEqual(
