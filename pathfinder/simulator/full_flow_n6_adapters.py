...
class N6PreparationLimits:
    ...
    max_total_frame_bytes: int = 1024 * 1024
...
def _validate_artifact(access: ArtifactAccess) -> None:
    ...
    _require(
        ...
        "exact range descriptor differs from its full artifact identity",
    )
    _require(
...
class N6ModelInputAdapter:
        ...
        *,
        ...
        if mode == "digest":
            request = self._digest_request(
                run_id, trial, stage, public_task, question, artifacts, identities
            )
    ...
    def _raw_request(
        ...
        sampled = tuple(self._raw_sampler(
            ...
            source_payload_sha256=access.payload_sha256,
            frame_count=self._limits.raw_frame_count,
            jpeg_max_dimension=self._limits.jpeg_max_dimension,
        ))
        _require(
            len(sampled) == self._limits.raw_frame_count,
    ...
    def _bundle_request(
        ...
        frames, sequence_sha = self._validated_bundle_frames(artifacts[0])
        return self._vision_request(
            run_id=run_id,
            trial=trial,
            stage=stage,
