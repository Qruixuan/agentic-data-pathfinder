...
from .cli_commands.full_flow_deployment import (
    dispatch_full_flow_deployment_command,
    register_full_flow_deployment_commands,
)
from .cli_commands.full_flow_semantic import (
    dispatch_full_flow_semantic_command,
    register_full_flow_semantic_commands,
)
from .cli_commands.full_flow_w4 import (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "verify-simulator-full-flow-semantic-matrix":
            ...
            return _print_payload(payload, compact=args.compact)
        full_flow_w4_status = dispatch_full_flow_w4_command(
            args,
            print_payload=_print_payload,
        )
        if full_flow_w4_status is not None:
            return full_flow_w4_status
        full_flow_semantic_status = dispatch_full_flow_semantic_command(
            args,
            print_payload=_print_payload,
            load_strict_json_file=_load_strict_json_file,
            load_n4_live_gate_sources=_load_n4_live_gate_sources,
            local_semantic_flowmesh_executor=(
                _local_semantic_flowmesh_executor
            ),
        )
        if full_flow_semantic_status is not None:
            return full_flow_semantic_status
        full_flow_deployment_status = dispatch_full_flow_deployment_command(
            args,
            print_payload=_print_payload,
        )
